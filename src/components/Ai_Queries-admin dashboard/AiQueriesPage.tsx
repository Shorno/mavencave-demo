...
const initialRows: QueryRow[] = [
  { id: "1", user: "সাদিয়া আক্তার", avatar: "/images/Ai_queries-Admin dashboard/user6.jpg", query: "কানাডায় কম্পিউটার সায়েন্সের সেরা বিশ্ববিদ্যালয়সমূহ", time: "২০-১০-২০২৫ রাত ৮:৩০", status: "সমাধান হয়েছে" },
  { id: "2", user: "মামিন দিশা", avatar: "/images/Ai_queries-Admin dashboard/user2.jpg", query: "আপনার বর্তমান প্রোফাইল দিয়ে F1 ভিসার সফলতার হার", time: "২০-১০-২০২৫ রাত ৮:২৪", status: "সমাধান হয়েছে" },
  { id: "3", user: "মিনহাজ হোসেন", avatar: "/images/Ai_queries-Admin dashboard/user3.jpg", query: "স্কলারশিপ অপশনসমূহ যুক্তরাজ্যে এমবিএ প্রোগ্রামসমূহ", time: "২০-১০-২০২৫ রাত ৭:৫৯", status: "সমাধান হয়েছে" },
  { id: "4", user: "আয়রিন জানিন", avatar: "/images/Ai_queries-Admin dashboard/user4.jpg", query: "জার্মানিতে ফাউন্ডেশন কোর্স এবং ভিসা যোগ্যতা", time: "২০-১০-২০২৫ রাত ৮:৪৮", status: "সমাধান হয়েছে" },
  { id: "5", user: "মারিনা তাসনিম", avatar: "/images/Ai_queries-Admin dashboard/user5.jpg", query: "৪.০ IELTS নিয়ে যুক্তরাজ্যের স্টুডেন্ট ভিসা কেমন সম্ভব?", time: "২০-১০-২০২৫ রাত ৭:৩৩", status: "ত্রুটি" },
  { id: "6", user: "রাইহান করিম", avatar: "/images/Ai_queries-Admin dashboard/user1.jpg", query: "যুক্তরাষ্ট্র ও কানাডায় বিশ্ববিদ্যালয়গুলোর তুলনা", time: "২০-১০-২০২৫ রাত ৭:২০", status: "সমাধান হয়েছে" },
];
...
const Avatar = ({ name, src }:{name:string; src?:string}) => {
  ...
  return (
    <div className={`h-9 w-9 bg-slate-400 text-white grid place-items-center rounded-full text-sm font-semibold`}>
      {initial}
    </div>
  );
};
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard label="মোট কুয়েরি" value={bn.format(2400)} delta="গতের চেয়ে ২.৮৭% বেশি" />
          <StatCard label="গড় সাড়া দেওয়ার সময়" value={`${bn.format(300)} মিলিসেকন্ড`} delta="গতের চেয়ে ২.৮৭% বেশি" />
          <StatCard label="কনভার্সন হার" value={`৩.৫%`} delta="গতের চেয়ে ২.৮৭% বেশি" />
        </div>

        {/* Card */}
        <div className="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-2xl font-semibold tracking-tight text-slate-900">AI কুয়েরি</h2>
          ...
          <SimpleDropdown
            ...
            items={[
              ...
              { key: "সমাধান হয়েছে", label: <><span className="h-2.5 w-2.5 rounded-full bg-emerald-500 inline-block"></span> সমাধান হয়েছে</> },
              { key: "ত্রুটি", label: <><span className="h-2.5 w-2.5 rounded-full bg-rose-500 inline-block"></span> ত্রুটি</> },
            ]}
          />
            ...
            <table className="min-w-full table-fixed text-left text-sm text-slate-700">
              <thead>
                <tr className="bg-indigo-50 text-slate-600">
                  {["ব্যবহারকারী","কুয়েরি","সময়","অবস্থা","কার্যক্রম"].map((h) => (
                ...
                {filteredRows.map((row, idx) => (
                  <tr key={row.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                    ...
                    <td className="relative px-6 py-4">
                      <button
                        ref={(el) => (rowStatusBtnRefs.current[row.id] = el)}
                        onClick={() => setOpenRowStatusFor(openRowStatusFor === row.id ? null : row.id)}
                        className={`inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-xs font-medium ${
                          row.status === "ত্রুটি" ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"
                        } hover:ring-2 ${row.status === "ত্রুটি" ? "hover:ring-rose-200 focus-visible:ring-rose-300" : "hover:ring-emerald-200 focus-visible:ring-emerald-300"} focus-visible:outline-none`}
                        aria-haspopup="menu"
                      ...
                      <SimpleDropdown
                        ...
                        anchorRef={{ current: rowStatusBtnRefs.current[row.id] as HTMLButtonElement }}
                        value={row.status}
                        onSelect={(v)=>setRowStatus(row.id, v as QueryStatus)}
                        onClose={()=>setOpenRowStatusFor(null)}
                        items={[
                          { key: "সমাধান হয়েছে" as QueryStatus, label: <><span className="h-2.5 w-2.5 rounded-full bg-emerald-500 inline-block"></span> সমাধান হয়েছে</> },
                          { key: "ত্রুটি" as QueryStatus, label: <><span className="h-2.5 w-2.5 rounded-full bg-rose-500 inline-block"></span> ত্রুটি</> },
                        ]}
                      />
                    </td>
                    <td className="relative px-6 py-4">
          ...
          <div className="mt-4 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            <p className="text-sm text-slate-500">
              {`${bn.format(2400)}টি AI কুয়েরির মধ্যে ১ থেকে ${bn.format(Math.min(filteredRows.length, 6))}টি দেখানো হচ্ছে`}
            </p>
